...
def get_reference_attn_matp(model, image_folder, block=-1, n_images=2000, device="cuda", attack_type=["FGSM", "PGD"], select_random=False, plot_path = "./plots/"):
    ...
    all_attns, mean_attns, mean_attn_diff = mean_attns_N_images(image_folder=image_folder, n_images=n_images, 
                                                                        block=block, model=model, n_random=select_random, device=device, attack_type=attack_type)
    ...
    for attack_name in attack_type:
        ...
        plt.savefig(os.path.join(plot_path, attack_name, f"mean_attns_{attack_name}_block_{block}_images_{n_images}.png"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, default='/home/aneeshashmi/xai/vit_base_patch16_224_in21k_test-accuracy_0.96_chest.pth', help='model checkpoint path')
    ...
    parser.add_argument('--train_path', type=str, default="/home/aneeshashmi/xai/data/real_data/train/", help='path to train data')
    parser.add_argument("--num_train_imgs", type=int, default=10, help="number of train images to use")
    parser.add_argument('--test_path', type=str, default="/home/aneeshashmi/xai/data/real_data/test/", help='path to test data')
    parser.add_argument("--dataset_class", type=str, default="Normal", choices=["tb", "normal"], help="dataset class")
    ...
    if args.force_recompute:
        ...
        for attack_name in args.attack + ['clean']:
            ...
            exits *= os.path.exists(os.path.join("./reference", "mean_images", f"mean_attns_{attack_name}_block_{args.block}_images_{args.num_train_imgs}.npy"))

    if exits:
        mean_attns = {}
        for attack_name in args.attack + ['clean']:
            mean_attns[attack_name] = np.load(os.path.join("./reference", "mean_images", f"mean_attns_{attack_name}_block_{args.block}_images_{args.num_train_imgs}.npy"))
        print(f'Loaded mean images from ./reference/mean_images/')
    
    all_attns, mean_attns, mean_attn_diff = get_reference_attn_matp(
        model = model,
        image_folder = os.path.join(args.train_path, "Normal"),
        block = args.block,
    ...
    for attack_name in args.attack + ['clean']:
        # mean_attns[attack_name] = mean_attns[attack_name].cpu().numpy()
        np.save(os.path.join("./reference", "mean_images", f"mean_attns_{attack_name}_block_{args.block}_images_{args.num_train_imgs}.npy"), mean_attns[attack_name])
    ...
    results_df.to_csv(f"./results/preds_{args.dataset_class}_block_{args.block}_images_{args.num_train_imgs}_eps_{args.eps}.csv", index=False)
