...
def get_reference_attn_map(model, image_folder, block=-1, n_images=2000, device="cuda", attack_type=["FGSM", "PGD"], select_random=False, plot_path = "./plots/", eps=0.03, random_state=0, transform=None):
    ...
    all_attns, mean_attns, mean_attn_diff = mean_attns_N_images(image_folder=image_folder, n_images=n_images, 
                                                                        block=block, model=model, n_random=select_random, device=device, attack_type=attack_type, eps = eps, random_state=random_state, transform=transform)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--attack', type=str, default='all', choices=['FGSM', "PGD", "all"], help='attack type (all for all) to perform on test sample')
    ...
    parser.add_argument("--num_train_imgs", type=int, default=1000, help="number of train images to use") # 2000
    parser.add_argument("--num_test_imgs", type=int, default=100, help="number of test images to use") # 500
    parser.add_argument("--dataset_class", type=str, default="Tuberculosis", choices=["Tuberculosis", "Normal"], help="dataset class") # Tuberculosis, Normal
    parser.add_argument("--block", type=int, default=-1, help="ViT block to take attention from")
    parser.add_argument("--eps", type=float, default=0.1, help="epsilon for adversarial attacks")
    ...
    parser.add_argument("--random_state", type=int, default=1, help="random state for experiments (train and test both)")
    parser.add_argument("--reference_attack", type=str, default= 'all', choices=['PGD', 'FGSM', "all"], help="attacks to test on")
    parser.add_argument("--test", default=True, choices=[True, False], help="Either to perform testing as well or just save and exit")
    # mean attacks
    ...
    if exits:
        ...
        all_attns, mean_attns, mean_attn_diff = get_reference_attn_map(
            model = model,
