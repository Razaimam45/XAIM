...
class VanillaVisionTransformer(VisionTransformer):
    ...
    def forward_features(self, x, return_attn=False):
        ...
        if return_attn:
            return [x[:, 0] for x in layer_wise_tokens], all_attn
        else:
    ...
    def forward(self, x, return_attn = False):
        if return_attn:
            list_out, all_attn = self.forward_features(x, return_attn=return_attn)
            return self.head(list_out[-1]), all_attn
        else:
            list_out = self.forward_features(x, return_attn=return_attn)
            return self.head(list_out[-1])
        # x = [self.head(x) for x in list_out]
        # return [out for out in x]
        # returning only the last head output (previously each output was returned--> error)
"""         y = 0
        for x in list_out:
            y = y + self.head(x)
        return y/12 """
