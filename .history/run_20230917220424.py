...
def classify_image(img_attn_map, mean_attn_clean, mean_attn_adv, method = 'all'): 
    ...
    if isinstance(mean_attn_adv, dict):
        ...
        if "euclidean" in method:
            ...
            euc_pred = "Clean"
            for key in attacks_used:
                sum_distance_to_adversarial = euclidean(test_attn_flat, mean_attn_adv[key].flatten())
                if euc_distance_to_normal < sum_distance_to_adversarial:
        ...
        if "cosine" in method:
            ...
            cos_pred = "Clean"
            for key in attacks_used:
                sum_distance_to_adversarial = cosine_similarity([test_attn_flat], [mean_attn_adv[key].flatten()])
                if cosine_distance_to_normal < sum_distance_to_adversarial:
