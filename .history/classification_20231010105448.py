        ...
        for selected_class in [0, 1]:
            ...
            for idx, img in tqdm(enumerate(val_data)):
                ...
                if pred.argmax() == selected_class:
                    ...
                    correctly_clf_imgs.append(img.squeeze(0).cpu().numpy())
            os.makedirs(args.save_path, exist_ok=True)
            os.makedirs(args.save_img_path, exist_ok=True)
            for img in correctly_clf_imgs:
                plt.imsave(os.path.join(args.save_img_path, f"{split}_{classes[selected_class]}_{correctly_clf_imgs.index(img)}.png"), img, cmap="gray"
                pass
