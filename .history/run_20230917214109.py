...
def get_images_attns(model, image_folder, n_imgs=20, block=-1, device="cuda", attack_type=['all'], eps=0.05, plot=False, n_random = False, random_state=None):
    ...
    image_files_clean = [f for f in os.listdir(os.path.join(image_folder, "Normal")) if f.endswith(".jpg") or f.endswith(".png")]

    image_files = image_files_adv + image_files_clean
