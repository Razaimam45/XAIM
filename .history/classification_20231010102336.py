...
if __name__ == "__main__":
    ...
    correctly_clf = []
    selected_class = 0
    val_data, val_img_names = get_data(os.path.join(args.data_path, "validation", classes[selected_class]), args.device)
    for idx, img in tqdm(enumerate(val_data)):
        ...
        break
    pd.DataFrame({"correctly_clf": correctly_clf}).to_csv(os.path.join(args.save_path, "correctly_clf.csv"), index=False)
